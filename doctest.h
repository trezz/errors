...
struct Expression_lhs
{
    ...
    DOCTEST_FORBIT_EXPRESSION(Expression_lhs, ^=)
    DOCTEST_FORBIT_EXPRESSION(Expression_lhs, |=)
    // these 2 are unfortunate because they should be allowed - they have higher precedence over the comparisons,
    // but the ExpressionDecomposer class uses the left shift operator to capture the left operand of the binary
    // expression...
    DOCTEST_FORBIT_EXPRESSION(Expression_lhs, <<)
    DOCTEST_FORBIT_EXPRESSION(Expression_lhs, >>)
};
...
struct DOCTEST_INTERFACE ExpressionDecomposer
{
    ...
    // table) but then there will be warnings from GCC about "-Wparentheses" and since "_Pragma()" is problematic
    // this will stay for now... https://github.com/catchorg/Catch2/issues/870
...
#define DOCTEST_TEST_CASE_CLASS(decorators)                                                                       \
    DOCTEST_CREATE_AND_REGISTER_FUNCTION_IN_CLASS(                                                                \
        DOCTEST_ANONYMOUS(_DOCTEST_ANON_FUNC_), DOCTEST_ANONYMOUS(_DOCTEST_ANON_PROXY_), decorators)
...
#define DOCTEST_TEST_CASE_FIXTURE(c, decorators)                                                                  \
    DOCTEST_IMPLEMENT_FIXTURE(                                                                                    \
        DOCTEST_ANONYMOUS(_DOCTEST_ANON_CLASS_), c, DOCTEST_ANONYMOUS(_DOCTEST_ANON_FUNC_), decorators)
    ...
    {                                                                                                             \
        ...
        {                                                                                                         \
            doctest::detail::regTest(                                                                             \
                doctest::detail::TestCase(func<Type>,                                                             \
                                          file,                                                                   \
                                          line,                                                                   \
...
#define DOCTEST_TEST_CASE_TEMPLATE_DEFINE(dec, T, id)                                                             \
    DOCTEST_TEST_CASE_TEMPLATE_DEFINE_IMPL(                                                                       \
        dec, T, DOCTEST_CAT(id, ITERATOR), DOCTEST_ANONYMOUS(_DOCTEST_ANON_TMP_))
...
#define DOCTEST_TEST_CASE_FIXTURE(x, name)                                                                        \
    DOCTEST_IMPLEMENT_FIXTURE(                                                                                    \
        DOCTEST_ANONYMOUS(_DOCTEST_ANON_CLASS_), x, DOCTEST_ANONYMOUS(_DOCTEST_ANON_FUNC_), name)
...
struct ConsoleReporter : public IReporter
{
    ...
    void test_run_end(const TestRunStats& p) override
    {
        ...
        auto totwidth = int(
            std::ceil(log10((std::max(p.numTestCasesPassingFilters, static_cast<unsigned>(p.numAsserts))) + 1)));
        ...
        s << Color::Cyan << "[doctest] " << Color::None << "test cases: " << std::setw(totwidth)
          ...
          << Color::None << " | " << (p.numTestCasesFailed > 0 ? Color::Red : Color::None) << std::setw(failwidth)
...
bool parseIntOption(int argc, const char* const* argv, const char* pattern, optionType type, int& res)
{
    ...
    if (type == 0) {
        ...
        // integer
        // TODO: change this to use std::stoi or something else! currently it uses undefined behavior - assumes '0'
        // on failed parse...
